...
{//start of namespace
    ...
    /// <summary>
    /// BotEngine - the central class that coordinates all bot features.
    /// MainWindow only calls HandleMessage() and OpeningMessage().
    ...
    {//start of class
        ...
        // true until the user has entered their name
        private bool _needsName = true;

        // keeps track of the most recently discussed topic for follow-up handling
        private string _recentTopic = string.Empty;

        // phrases that mean the user wants more info on the same topic
        ...
        // FetchName - used by MainWindow to update the status bar
        ...
        // HandleMessage - the single entry point called by MainWindow.
        // Processes the input through every feature in the correct order.
        ...
        {//start of HandleMessage
            ...
            // STEP 1: Capture the user's name on the very first message
            if (_needsName)
            {
            ...
            // STEP 2: Follow-up handling - give another tip on the same topic
            ...
            // STEP 3: Interest declaration - store and immediately respond
            ...
            if (!string.IsNullOrEmpty(topic_answer))
            {
                ...
                // if a mood was detected, prepend the empathetic opener then give the tip
            ...
            // if mood detected but no keyword matched, give opener + a random tip
            if (!string.IsNullOrEmpty(mood_opener))
            {
        ...
        // PullTopicFromSentence - extracts the topic after "interested in", "I like" etc.
        ...
        // BuildTopicList - returns the full help listing
        ...
        // PickFallback - returns a random fallback message when nothing matched
